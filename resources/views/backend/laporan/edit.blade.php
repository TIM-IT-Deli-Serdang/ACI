...
<div class="fv-row mb-7">
    ...
    <select name="kategori_laporan_id" class="form-select form-select-solid" data-control="select2" data-hide-search="true"
...
<div class="fv-row mb-7">
    ...
    <textarea name="deskripsi" class="form-control form-control-solid" rows="3"
...
<div class="row mb-7">
    ...
    <div class="col-md-6 fv-row">
        ...
        <select id="edit_kecamatan" name="kecamatan_id" class="form-select form-select-solid" data-control="select2"
            data-placeholder="Pilih Kecamatan" data-old="{{ $data['kecamatan_id'] ?? '' }}">
            <option></option>
        </select>
    </div>
    ...
    <div class="col-md-6 fv-row">
        ...
        <select id="edit_kelurahan" name="kelurahan_id" class="form-select form-select-solid" data-control="select2"
            data-placeholder="Pilih Kelurahan" data-old="{{ $data['kelurahan_id'] ?? '' }}">
            <option></option>
        </select>
    </div>
</div>
...
<div class="fv-row mb-7">
    ...
    <textarea name="alamat" class="form-control form-control-solid" rows="2"
...
<div class="row mb-7">
    <div class="col-md-5 fv-row">
        ...
        <input type="text" id="edit_lat" name="latitude" class="form-control form-control-solid"
            placeholder="-3.xxxx" value="{{ $data['latitude'] ?? '' }}" />
    </div>
    <div class="col-md-5 fv-row">
        <label class="fw-semibold fs-7 mb-2">Longitude</label>
        <input type="text" id="edit_long" name="longitude" class="form-control form-control-solid"
            placeholder="98.xxxx" value="{{ $data['longitude'] ?? '' }}" />
    </div>
    <div class="col-md-2 d-flex align-items-end">
        <button type="button" id="btn-get-loc-edit" class="btn btn-icon btn-light-primary w-100"
            ...
            if (res.data) {
                res.data.forEach(kec => {
                    ...
                    $('#edit_kecamatan').append(
                        `<option value="${kec.id}" ${isSelected}>${kec.nama}</option>`);
                });
            }

            // Jika ada oldKec, trigger load kelurahan
            if (oldKec) {
            ...
            $.getJSON(url, function(res) {
                ...
                if (res.data) {
                    res.data.forEach(kel => {
                        ...
                        $('#edit_kelurahan').append(
                            `<option value="${kel.id}" ${isSelected}>${kel.nama}</option>`);
                    });
                }
            });
        }
