                        ...
                        <div class="mb-5 text-center">
                            ...
                            @if($fotoUrl)
                                <a href="{{ $fotoUrl }}" target="_blank">
                                    <img src="{{ $fotoUrl }}" class="rounded mw-100 shadow-sm" style="max-height: 300px; object-fit: cover;" alt="Bukti Laporan">
                                </a>
                            @else
                            ...
                            <table class="table table-flush fw-semibold gy-1">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-gray-800">
                                            @switch($data['kategori_laporan_id'] ?? 0)
                                                @case(1) Jalan Rusak @break
                                                @case(2) Drainase Tersumbat @break
                                                @case(3) Banjir @break
                                                @case(4) Jembatan Rusak @break
                                                @case(5) Infrastruktur Lain @break
                                                @default -
                                            @endswitch
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="text-gray-800">{{ \Carbon\Carbon::parse($data['created_at'])->translatedFormat('d F Y, H:i') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted">Lokasi</td>
                                        <td class="text-gray-800">
                                            {{ $data['alamat'] }} <br>
                                            <small class="text-muted">Lat: {{ $data['latitude'] }}, Long: {{ $data['longitude'] }}</small>
                                            @if($data['latitude'] && $data['longitude'])
                                                <a href="https://maps.google.com/?q={{ $data['latitude'] }},{{ $data['longitude'] }}" target="_blank" class="ms-2 badge badge-light-primary">Lihat Peta</a>
                                            @endif
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <div class="card-body">
                        <div class="timeline-label">
                            ...
                            <div class="timeline-item">
                                <div class="timeline-label fw-bold text-gray-800 fs-6">{{ \Carbon\Carbon::parse($data['created_at'])->format('H:i') }}</div>
                            ...
                            @if(!empty($data['penerima_tgl']))
                                <div class="timeline-item">
                                    <div class="timeline-label fw-bold text-gray-800 fs-6">{{ \Carbon\Carbon::parse($data['penerima_tgl'])->format('H:i') }}</div>
                                    <div class="timeline-badge">
                            ...
                            @if(!empty($data['verif_tgl']))
                                <div class="timeline-item">
                                    <div class="timeline-label fw-bold text-gray-800 fs-6">{{ \Carbon\Carbon::parse($data['verif_tgl'])->format('H:i') }}</div>
                                    <div class="timeline-badge">
                            ...
                            @if(!empty($data['penanganan_tgl']))
                                <div class="timeline-item">
                                    <div class="timeline-label fw-bold text-gray-800 fs-6">{{ \Carbon\Carbon::parse($data['penanganan_tgl'])->format('H:i') }}</div>
                                    <div class="timeline-badge">
                            ...
                            @if(!empty($data['selesai_tgl']))
                                <div class="timeline-item">
                                    <div class="timeline-label fw-bold text-gray-800 fs-6">{{ \Carbon\Carbon::parse($data['selesai_tgl'])->format('H:i') }}</div>
                                    <div class="timeline-badge">
                ...
                <div class="card card-flush h-lg-100">
                        ...
                        <h3 class="card-title align-items-start flex-column">
                            ...
                            <span class="text-gray-500 mt-1 fw-semibold fs-6">
                                ...
                                @switch($data['status_laporan'])
                                    ...
                                    @case(2) <span class="badge badge-primary">Diverifikasi</span> @break
                                    @case(3) <span class="badge badge-warning">Penanganan</span> @break
                                    @case(4) <span class="badge badge-success">Selesai</span> @break
                                    @case(5) <span class="badge badge-danger">Ditolak</span> @break
                                @endswitch
                            </span>
                        </h3>
                    </div>
                    ...
                    <div class="card-body">
                        @php
                            ...
                            elseif ($status == 1 && ($isSuperAdmin || $isUpt)) $showForm = true;
                            elseif ($status == 2 && ($isSuperAdmin || $isSda)) $showForm = true;
                            elseif ($status == 3 && ($isSuperAdmin || $isSda)) $showForm = true;
                            
                            if ($status == 4 || $status == 5) $showForm = false;
                        @endphp

                        @if($showForm)
                            <form id="form_validasi" action="{{ route('laporan.process-validation', $data['id']) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                        ...
                        @else
                            <div class="alert alert-dismissible bg-light-primary border border-primary d-flex flex-column flex-sm-row w-100 p-5 mb-10">
                                ...
                                <div class="d-flex flex-column pe-0 pe-sm-10">
                                    ...
                                    <span>
                                        @if($status == 4) Laporan ini sudah <b>Selesai</b>.
                                        @elseif($status == 5) Laporan ini sudah <b>Ditolak</b>.
                                        @else Anda tidak memiliki hak akses untuk memproses status ini. @endif
                                    </span>
                                </div>
                            </div>
                        @endif
